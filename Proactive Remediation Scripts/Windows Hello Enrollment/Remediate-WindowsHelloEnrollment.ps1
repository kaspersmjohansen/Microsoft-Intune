<#
.SYNOPSIS
    Script will remove the Windows Hello for Business credentials container.
...
.DESCRIPTION
    ...
    If the logged on user is not making use of the PIN credential provider, the script will exit with error 1 and execute the certutil.exe -deleteHelloContainer command
    which will reset the Windows Hello for Business credentials container forcing af reconfiguration of the Windows Hello for Business PIN.

    Credit goes to Martin Bengtsson who is the author of the Detect-WindowsHelloEnrollment script, which can be found here:
    https://www.imab.dk/endpoint-analytics-find-devices-not-enrolled-with-windows-hello-for-business/

    The original script has been repurposed to do remediation and not only detection.

.NOTES
    Filename: Remediate-WindowsHelloEnrollment.ps1
    Version: 1.0
    Author: Kasper Johansen
    Blog: https://kasperjohansen.net
    Twitter: @kaspermjohansen
